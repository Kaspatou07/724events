...
const Select = ({
  ...
  const [collapsed, setCollapsed] = useState(true); // État pour contrôler si la liste est affichée ou non

  const changeValue = (newValue) => {
    onChange(newValue); // Informe le composant parent de la nouvelle sélection
    setValue(newValue); // Met à jour la valeur sélectionnée dans l'interface
  ...
  return (
    <div className={`SelectContainer ${type}`} data-testid="select-testid">
      ...
      <div className="Select">
        <ul>
          <li className={collapsed ? "SelectTitle--show" : "SelectTitle--hide"} onClick={() => setCollapsed(!collapsed)}>
